...
class ScheduleRepository:
    ...
    def get(self, schedule_id: int) -> object | None:
        ...
        return self._session.scalars(
            select(self._model).where(
                self._model.id == schedule_id,
                self._model.deleted_at.is_(None),
            )
        ).first()

    def get_including_deleted(self, schedule_id: int) -> object | None:
        """Return a schedule by id regardless of deletion state."""
        return self._session.get(self._model, schedule_id)

    def list_between(self, start_at: int, end_at: int) -> list[object]:
        """Return non-deleted schedules that overlap a query window."""
        statement = (
            select(self._model)
            .where(
                self._model.start_at < end_at,
                self._model.end_at > start_at,
                self._model.deleted_at.is_(None),
            )
    ...
    def find_conflict(
        ...
        """Return the first non-deleted schedule that overlaps the given window."""
